...
import saveFestival from "@salesforce/apex/ganttChart.saveFestival";
...
export default class GanttChart extends LightningElement {
  ...
  OpenFestivoModal() {
    ...
    this.template.querySelector(".create-festivo-modal").show();
  }
...
*/
  ...
  guardarFestivo() {
    const startDate = new Date(this.diasFestivos.startDate  + "T00:00:00");
    const endDate = new Date(this.diasFestivos.endDate + "T00:00:00");
    saveFestival({
      ...
      this.diasFestivos = {};
      this.template.querySelector(".create-festivo-modal").hide();
    })
    .catch(error => {
      this.dispatchEvent(
        new ShowToastEvent({
 ...
 DiaCambiado(event){
    ...
    this.diasFestivos[event.target.dataset.field]=event.target.value;
