...
export default class GanttChartResource extends LightningElement {
  ...
  setProjects() {
    ...
    Object.keys(self._resource.allocationsByProject).forEach(projectId => {
      ...
      self.resource.allocationsByProject[projectId].forEach(alloc => {        
        //console.warn(alloc);

        let allocation = {
          class : self.calcClass(alloc),
          style : self.calcStyle(alloc),
          labelStyle : self.calcLabelStyle(alloc),
          projectName : alloc.projectName,
          Effort__c : alloc.Effort__c,
          End_Date__c : alloc.End_Date__c,
          Id : alloc.Id,
          Dedication__c:alloc.Dedication__c,
          Start_Date__c : alloc.Start_Date__c,
          Status__c : alloc.Status__c,
          color : alloc.color,
          left : alloc.left,
          right : alloc.right
        };        
  ...
  addAllocationModalSuccess() {
    ...
    this._saveAllocation({
      ...
      dedication:this.addAllocationData.dedication
    })
      .then(() => {
  ...
  handleModalEditClick(event) {
    ...
    this.editAllocationData = {
      ...
      dedication:this.menuData.allocation.Dedication__c,
  ...
  editAllocationModalSuccess() {
    ...
    this._saveAllocation({
      ...
      dedication:this.editAllocationData.dedication
    })
      .then(() => {
        this.editAllocationData = {};
